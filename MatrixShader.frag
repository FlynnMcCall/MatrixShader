...
// background Colour
const vec3 backCol = vec3(0.005,0.005,0.005);
...
const vec3 lowColRange = vec3(0.048,0.690,0.115);
const vec3 highColRange = vec3(0.108,0.990,0.095);



const float drawCharCutttoff = 0.364;
const float charNoiseCuttoff = 0.476;
const int charVertBuffer = 2;
...
const float numColumns = 30.024;



const float textHbuffer = 0.320;
const vec2 charDimensions = vec2(3,18);
const float pixelAR = 1.0;
const float updatesPerSec = 10.624;
...
void main() {
    ...
    // ^^Colour Buffer backcol
        ...
        float columnRand = random(vec2(getColumn(st.x, numColumns)* numColumns, 8.9));
        // a unique random number shared by a whole column
        

        int timeFunct = -int(u_time * updatesPerSec + 10.0 * columnRand);
        ...
        if (NotOnBuffer(intColumn, intRow + timeFunct)){
            ...
            if (random(vec2(float(intColumn+ timeFunct + int(columnRand *10.0)) , columnRand*float(intRow + timeFunct))) > charNoiseCuttoff){
                
                vec2 commonCharVec = vec2(15.2 * columnRand , float(intRow + timeFunct) - mod(float(intRow + timeFunct), float(charVertBuffer) + charDimensions.y));
                ...
                if (random(commonCharVec) > drawCharCutttoff){
                	...
                	float tone = random(vec2(commonCharVec.y + float(timeFunct),commonCharVec.y ));
